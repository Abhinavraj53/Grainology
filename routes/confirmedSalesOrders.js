...
function buildListQuery(params) {
  ...
  addTextFilter(query, 'seller_name', params.party);
...
router.get('/', requireAdmin, async (req, res) => {
  try {
    ...
    const [orders, total, pending, approved, declined] = await Promise.all([
      ConfirmedSalesOrder.find(query)
        .select(LIST_ORDER_SELECT)
        .populate('customer_id', 'name email mobile_number')
        .populate('created_by', 'name email')
        .sort({ createdAt: -1 })
        ...
        .limit(limit)
        .lean(),
      ConfirmedSalesOrder.countDocuments(query),
      ConfirmedSalesOrder.countDocuments({ ...query, approval_status: 'pending' }),
      ConfirmedSalesOrder.countDocuments({ ...query, approval_status: 'approved' }),
      ConfirmedSalesOrder.countDocuments({ ...query, approval_status: 'declined' }),
    ]);
