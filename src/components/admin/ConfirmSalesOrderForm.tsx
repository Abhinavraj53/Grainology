...
export default function ConfirmSalesOrderForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!customerId) {
        showError('Please select a seller name');
        setSubmitting(false);
        return;
      }
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const sampleRow1 = [
                        ...
                        sampleState,
                        sampleSeller1, // Customer name
                        sampleSeller1, // Seller Name (same as customer)
                        sampleLocation,
                        sampleWarehouse,
                        '16',
                        sampleCommodity1,
                    ...
                    try {
                      ...
                      const buffer = await generateSampleExcel({
                        ...
                        dropdownColumns: [
                          ...
                          { columnIndex: 3, formula: 'Customers' },   // Customer (C)
                          { columnIndex: 4, formula: 'Customers' },    // Seller Name (D)
                          { columnIndex: 5, formula: 'Locations' },   // Location (E) - fixed list, opens in all Excel
                          { columnIndex: 6, formula: 'Warehouses' }, // Warehouse (F) - fixed list, opens in all Excel
                          { columnIndex: 8, formula: 'Commodities' }, // Commodity (H)
                          { columnIndex: 9, formula: 'Varieties' },  // Variety (I) - fixed list, opens in all Excel
                        ],
                      });
