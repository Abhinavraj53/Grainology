...
export default function PurchaseOrderHistory({ userId, userName }: PurchaseOrderHistoryProps) {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/purchase-orders?buyer_id=${userId}`, {
        headers: {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">View all your purchase orders and their status</p>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">No Purchase Orders Yet</h2>
          <p className="text-gray-600">Start by creating a new purchase order using the Create Trade option.</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">Rate (MT)</th>
                ...
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900 font-bold">
                      ₹{((order.expected_price_per_quintal || 0) * 10).toLocaleString()}
                    </td>
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <p className="text-lg text-gray-900 font-bold">₹{((selectedOrder.expected_price_per_quintal || 0) * 10).toLocaleString()}</p>
                </div>
                <div>
